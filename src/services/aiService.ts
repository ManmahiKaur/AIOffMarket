...
function buildLocationResponse(
  ...
  const signals: AIKeySignal[] = [
    {
      ...
      source: 'Supabase Property Database',
    },
    {
      id: 'sig-loc-2',
  ...
  if (avgPrice > 0) {
    signals.push({
      ...
      source: 'Supabase Valuation Data',
    });
  }
  ...
  return {
    ...
    modelUsed: 'Supabase Database Search + Rule Engine',
    locationContext: {
...
function buildNoResultsResponse(query: string, searchedLocation: string, totalPropertiesCount: number): AIQueryResponse {
  ...
  return {
    ...
    explanation: `The search for "${loc}" returned zero matches across all ${totalPropertiesCount.toLocaleString()} property records in Supabase. This could be because the location name is spelled differently in the database, or there are no tracked properties in that area yet.`,
    opportunityScore: 0,
    priority: 'LOW',
    confidenceScore: 99,
    confidenceLevel: 'VERY HIGH',
    ...
    keySignals: [{
      ...
      source: 'Supabase Database Search',
    }],
    topProperties: [],
    recommendedActions: [{
      step: 1,
    ...
    modelUsed: 'Supabase Database Search',
...
export const aiService = {
  ...
  async processNaturalLanguageQuery(userQuery: string, targetPropertyId?: string): Promise<AIQueryResponse> {
    ...
    // 1. Fetch live data from Supabase (cached)
      ...
      if (matches.length > 0) {
        ...
        if (isGroqConfigured() && matches.length > 0) {
          try {
            ...
            response.modelUsed = 'Supabase Database + Groq LLaMA 3.3 70B';
          } catch {
...
Write a 3-4 sentence direct answer summarising what was found and the top investment opportunity.`;
  ...
  generateDeterministicResponse(
    ...
    if (isSellerMotivation) {
      const motivatedOpps = allOpportunities
      ...
      return {
        ...
        directAnswer: `Our engine identified ${motivatedOpps.length} prime properties demonstrating high vendor motivation across your monitored database. Key drivers include probate estate liquidations, prolonged vacancy (60+ days), and withdrawn listings.`,
        explanation: `When property owners face forced timelines — estate settlements, vacant property holding costs, or failed campaigns — their price flexibility increases 8–15%. These properties represent asymmetric off-market targets.`,
        opportunityScore: 91,
        priority: 'CRITICAL',
        confidenceScore: 96,
        confidenceLevel: 'VERY HIGH',
        sellerMotivation: {
          ...
          keyDrivers: [
            ...
            'Withdrawn Public Listings: Vendor fatigue after failed campaigns',
          ],
        },
        keySignals: [
          {
