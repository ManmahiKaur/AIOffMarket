import type { Opportunity, Property, PropertyEvent } from '../types';
import { liveDb } from '../lib/dbClient';
import { mapDatabaseOpportunity, mapDatabaseProperty, mapDatabaseEvent } from '../lib/mapping';

export interface ResolvedOpportunity extends Opportunity {
  property: Property;
  event: PropertyEvent;
}

export interface OpportunityFilters {
  eventTypes?: string[];
  minScore?: number;
}

export const opportunityService = {
  async getOpportunities(filters?: OpportunityFilters): Promise<ResolvedOpportunity[]> {
    let query = liveDb
      .from('opportunity_scores')
      .select('*')
      .order('score', { ascending: false });

    if (filters?.minScore !== undefined) {
      query = query.gte('score', filters.minScore);
    }
    
    // 1. Fetch scores from live database
    const { data: scores, error: scoreErr } = await query.range(0, 2999);
      
    if (scoreErr || !scores || scores.length === 0) return [];
    
    // 2. Fetch associated properties and events
    const propertyIds = scores.map(s => s.property_id);
    const { data: dbProperties } = await liveDb
      .from('properties')
      .select('*')
      .in('id', propertyIds);
      
    const { data: dbEvents } = await liveDb
      .from('events')
      .select('*')
      .in('property_id', propertyIds);
      
    // 3. Fetch score_factors
    const scoreIds = scores.map(s => s.id);
    const { data: factors } = scoreIds.length > 0
      ? await liveDb.from('score_factors').select('*').in('score_id', scoreIds)
      : { data: [] };

    let resolvedOpportunities: ResolvedOpportunity[] = [];
    const props = (dbProperties || []).map(mapDatabaseProperty);
    
    for (const score of scores) {
      // Find latest event for this property (or create synthetic event)
      const propEvents = (dbEvents || []).filter(e => e.property_id === score.property_id);
      const latestEvent = propEvents.sort((a, b) => new Date(b.detected_at).getTime() - new Date(a.detected_at).getTime())[0]
        || { id: `evt-${score.property_id}`, property_id: score.property_id, event_type: 'NEW_LISTING', detected_at: score.computed_at || new Date().toISOString() };
      
      const prop = props.find(p => p.id === score.property_id.toString());
      
      if (!prop) continue; 
      
      // Apply event type filter
      if (filters?.eventTypes && filters.eventTypes.length > 0) {
        if (!filters.eventTypes.includes(latestEvent.event_type)) {
          continue;
        }
      }
      
      const scoreFactors = (factors || []).filter(f => f.score_id === score.id);
      
      const baseOpp = mapDatabaseOpportunity(latestEvent, score, null, scoreFactors);
      const mappedEvent = mapDatabaseEvent(latestEvent);
      
      resolvedOpportunities.push({
        ...baseOpp,
        property: prop,
        event: mappedEvent
      });
    }
    
    // limit to 20 after filtering
    return resolvedOpportunities.slice(0, 20);
  },

  async getOpportunityById(id: string): Promise<Opportunity | undefined> {
    const { data: events } = await liveDb.from('events').select('*').eq('id', id).limit(1);
    if (!events || events.length === 0) return undefined;
    const event = events[0];
    
    const { data: scores } = await liveDb.from('opportunity_scores').select('*').eq('property_id', event.property_id).order('computed_at', { ascending: false }).limit(1);
    const score = scores?.[0];
    
    const { data: explanations } = await liveDb.from('ai_explanations').select('*').eq('event_id', event.id).limit(1);
    const explanation = explanations?.[0];
    
    const { data: factors } = score 
      ? await liveDb.from('score_factors').select('*').eq('score_id', score.id) 
      : { data: [] };
    
    return mapDatabaseOpportunity(event, score, explanation, factors || []);
  },

  async getOpportunitiesByPropertyId(propertyId: string): Promise<Opportunity[]> {
    const { data: events } = await liveDb.from('events').select('*').eq('property_id', propertyId);
    if (!events || events.length === 0) return [];
    
    const { data: scores } = await liveDb.from('opportunity_scores').select('*').eq('property_id', propertyId);
    
    const eventIds = events.map(e => e.id);
    const { data: explanations } = eventIds.length > 0 
      ? await liveDb.from('ai_explanations').select('*').in('event_id', eventIds)
      : { data: [] };
      
    const scoreIds = (scores || []).map(s => s.id);
    const { data: factors } = scoreIds.length > 0 
      ? await liveDb.from('score_factors').select('*').in('score_id', scoreIds) 
      : { data: [] };

    return events.map(event => {
      const score = (scores || []).sort((a, b) => new Date(b.computed_at).getTime() - new Date(a.computed_at).getTime())[0];
      const explanation = (explanations || []).find(e => e.event_id === event.id);
      const scoreFactors = score ? (factors || []).filter(f => f.score_id === score.id) : [];
      return mapDatabaseOpportunity(event, score, explanation, scoreFactors);
    });
  }
};
